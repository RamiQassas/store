...
def wipe_provider_catalog(apps, schema_editor):
    ...
    # 3. Also find any API product variants not in mappings (by api_product_id)
    api_variant_ids = list(
        ProductVariant.objects.filter(api_product_id__isnull=False).exclude(api_product_id="")
    ...
    # 4. Delete mappings first (FK constraints)
    ...
    # Also catch any with is_api_product=True that still exist
    ...
    print("[0040] Done. Run sync from admin panel to re-import.")
